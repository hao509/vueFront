          ...
          <template slot-scope="{ row }">
            <el-image style="width: auto; height: 40px; border:none;cursor: pointer;" :src="getImage(row.image)" :preview-src-list="[ `/common/download?name=${row.image}` ]" >
              <div slot="error" class="image-slot">
                <img src="../../styles/images/noImg.png"  style="width: auto; height: 40px; border:none;" >
              </div>
            </el-image>
          </template>
        </el-table-column>
...
<script>
import { getSetmealPage, deleteSetmeal, setmealStatusByStatus } from '../../utils/api/combo'
export default {
  name: 'ComboList',
  data () {
    return {
  ...
  methods: {
    ...
    addSetMeal (st) {
      if (st === 'add') {
        ...
        this.$router.push(`/comboadd?id=${st}`)
        // window.parent.menuHandle({
        //   id: '5',
